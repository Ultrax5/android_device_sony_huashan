...
$(INSTALLED_BOOTIMAGE_TARGET): $(PRODUCT_OUT)/kernel $(uncompressed_ramdisk) $(recovery_uncompressed_ramdisk) $(INSTALLED_RAMDISK_TARGET) $(INITSH) $(BOOTREC_DEVICE) $(PRODUCT_OUT)/utilities/busybox $(PRODUCT_OUT)/utilities/extract_elf_ramdisk $(MKBOOTIMG) $(MINIGZIP) $(INTERNAL_BOOTIMAGE_FILES)
	...
	$(hide) cd $(PRODUCT_OUT)/root && chmod 750 `ls $(PRODUCT_OUT)/root` && chmod 755 $(PRODUCT_OUT)/root/data && chmod 755 $(PRODUCT_OUT)/root/dev && chmod 755 $(PRODUCT_OUT)/root/proc && chmod 755 $(PRODUCT_OUT)/root/sbin && chmod 755 $(PRODUCT_OUT)/root/sys && chmod 755 $(PRODUCT_OUT)/root/system && tar -cvf ramdisk.tar `ls $(PRODUCT_OUT)/root`
